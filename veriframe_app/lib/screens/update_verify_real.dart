...
void main() {
  ...
  // Update VerifyPage constructor
  final classStart = content.indexOf('class VerifyPage extends StatefulWidget {');
  final classEnd = content.indexOf('class _VerifyPageState extends State<VerifyPage>');
  if (classStart != -1 && classEnd != -1) {
    String newClass = '''class VerifyPage extends StatefulWidget {
...
''';
    content = content.replaceRange(classStart, classEnd, newClass);
  }
  ...
  static const String _baseUrl = 'http://10.0.2.2:3000'; // Change to actual backend IP if testing on real device
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          if (data['detected_indicators'] != null) {
            _detectedIndicators = List<String>.from(data['detected_indicators']);
          }
      ...
      setState(() {
        ...
        if (_errorMessage!.contains('weights not installed')) {
          _errorMessage = 'EfficientViT/CrossEfficientViT weights not installed.';
        } else if (_errorMessage!.contains('Connection refused') || _errorMessage!.contains('Failed host lookup')) {
          _errorMessage = 'AI analysis service unavailable.';
        }
      });
    }
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Analysis Failed',
            style: const TextStyle(color: _danger, fontSize: 16, fontWeight: FontWeight.bold),
          ),
          const SizedBox(height: 8),
          Text(
            _errorMessage!,
    ...
    return _Card(
      child: Column(
        ...
        children: [
          ...
          const Text(
            'The AI identified the following indicators during frame-by-frame forensic analysis:',
          ...
          if (_detectedIndicators.isNotEmpty)
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: _detectedIndicators.map((ind) => _buildIndicatorChip(ind)).toList(),
            )
          else
            const Text('No notable manipulation indicators detected.', style: TextStyle(color: _safe, fontSize: 13)),
          ...
          if (_explanation.isNotEmpty)
            Container(
              ...
              child: Text(
                'AI Explanation: \$_explanation',
                style: const TextStyle(color: _textPri, fontSize: 13, height: 1.5, fontStyle: FontStyle.italic),
              ),
            ),
          
          if (_manipulatedScore >= 80) ...[
            const SizedBox(height: 16),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
  ...
  Widget _buildIndicatorChip(String label) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(label, style: const TextStyle(color: _danger, fontSize: 11)),
        ],
      ),
    );
  }
